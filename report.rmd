...
The plot also includes terms such as age_group.L, age_group.Q, and higher-order contrasts. These represent overall age trends rather than specific age groups. Their significance indicates that age-related patterns still influence satisfaction, complementing the effects of individual service ratings. 

We include quadratic (Q) and higher-order polynomial terms because the relationship between age and satisfaction may not be linear. For instance, satisfaction might peak in middle age groups (creating a quadratic pattern) or exhibit more complex S-shaped patterns across the lifespan. A linear term alone would miss these nuanced age effects, potentially leading to incomplete understanding of how age moderates service satisfaction. By testing multiple polynomial contrasts, we can capture whether age effects follow simple monotonic trends or more complex nonlinear patterns that better reflect real-world heterogeneity in passenger preferences across different life stages.

Overall, the figure highlights both key service drivers of satisfaction and the continued role of demographic age trends.
...
Figure 3.2.2 displays scatter plots with fitted regression lines for entertainment and catering ratings against flight distance. Each point is an individual passenger, and the red dashed line is the fitted linear trend; its light band is the 95% confidence interval. A steeper upward red line indicates stronger positive association between distance and the rating, while a flat line indicates little relationship. The base models (without controls) show positive slopes, indicating that longer flights are associated with higher ratings. However, this relationship needs to be examined more carefully after controlling for other factors.
...
The key results from the **full regression models** (with controls) are summarized in Figure 3.2.3, which shows the coefficient estimates for flight distance along with 95% confidence intervals. The red dashed horizontal line marks a coefficient of zero—values above it imply higher ratings as distance grows; values below imply lower ratings; bars whose confidence intervals cross the line are not statistically different from zero. For entertainment ratings, the coefficient is `r sprintf("%.6f", ent_coef)` (p = `r sprintf("%.3f", ent_pval)`), which is **not statistically significant** at conventional levels (p > 0.05). This suggests that after controlling for passenger characteristics and other service dimensions, flight distance does not have a significant effect on entertainment ratings.
...
The logistic regression with interaction terms reveals pronounced heterogeneity in how business and personal travelers translate time and comfort into satisfaction. Figure 3.3.4 displays the odds ratios for schedule convenience and onboard comfort effects, separately for business and personal travelers. The figure shows that business travelers exhibit substantially larger odds ratios for both attributes compared to personal travelers, with comfort effects being particularly pronounced.
