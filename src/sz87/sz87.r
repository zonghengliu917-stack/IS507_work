...
# 选取服务相关评分列 ---------------------------------------------------------
# 评分范围通常为1-5，数据中存在0（表示未评价/不可用），这里将0视作缺失
service_columns <- c(
...
heatmap_plot <- ggplot(summary_scores, aes(x = service, y = age_group, fill = mean_score)) +
  geom_tile(color = "white", size = 0.2) + # 使用 size 兼容旧版 ggplot2
...
# K–S 检验：对比两个年龄段在各服务的评分分布 -----------------------------------
# 默认选择最年轻与最年长两个年龄段（若都存在）
...
# 逻辑回归：满意度 ~ 年龄段 + 各服务评分 --------------------------------------
# 将满意度二值化：satisfied = 1，其他 = 0；并移除服务列中的0（视作缺失）
logit_df <- df |>
  dplyr::mutate(
...
if (nrow(logit_df) > 100) {
  # 构建公式：satisfaction_binary ~ age_group + service1 + service2 + ...
